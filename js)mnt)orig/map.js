...
function checkEmpty(id){
  ...
  var empty=$.trim(text).length===0;
  if(empty){
    $elem.addClass('invalid_contact');
  }
  return !empty;
}
function validContactForm(){
  var valid=true;
  valid= valid && checkEmpty('contact_email');
  valid= valid && checkEmpty('contact_message');
...
$(window).load(function(){ // after loading the DOM
    $("#ajax_contact_form").submit(function(){
      if(!validContactForm()){
        $("#contact_status").text("Please fill the mandatory fields");
        return false;
      }
